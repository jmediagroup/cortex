...
import { 
  Zap, Calculator, TrendingDown, Scale, ShieldCheck, DollarSign, Info 
} from 'lucide-react';
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" value={profit} onChange={(e) => setProfit(parseFloat(e.target.value) || 0)} className="w-full pl-8 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold outline-none" />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  <span className="absolute left-4 top-2.5 text-slate-300 font-bold">$</span>
                  <input type="number" value={salary} onChange={(e) => setSalary(parseFloat(e.target.value) || 0)} className="w-full pl-8 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold outline-none" />
                </div>
              </div>
            </div>
          ...
          <div className="h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={[
                ...
                <Tooltip cursor={{fill: '#f8fafc'}} formatter={(v: number | undefined) => `$${Math.round(v ?? 0).toLocaleString()}`} />
