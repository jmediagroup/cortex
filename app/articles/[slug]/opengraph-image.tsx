...
// Per-article social card. Static params come from the page's
// generateStaticParams, so every slug gets a real branded image at build time
// for shares on X/LinkedIn/iMessage (and AI answer-engine citations). This is
// the standard Money Guy Mutant fallback used whenever an article has no
// uploaded featured image.

export const alt = 'Money Guy Mutants — Financial Insights & Guides';
export const size = {
  width: 1200,
  height: 630,
};
...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const title = article?.title ?? 'Financial Insights';
  const date = article ? formatDate(article.date) : '';
  const label = article?.categories[0]?.name ?? 'ARTICLE';
  const tags = article?.tags.slice(0, 5).map((t) => t.name) ?? [];
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        >
          <span>{date}</span>
          <span style={{ letterSpacing: '2px', fontWeight: 700, color: BRAND.sky }}>MONEYGUYMUTANTS.COM/ARTICLES</span>
        </div>
      </div>
    ),
    {
