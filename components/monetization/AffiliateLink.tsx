  ...
  if (variant === 'inline') {
    return (
      <a
        ...
        onClick={handleClick}
        className={`text-[var(--emerald-400)] hover:text-[var(--mist-200)] underline underline-offset-2 ${className}`}
      >
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          ...
          {affiliate.tagline && (
            <p className="text-sm text-[var(--emerald-400)] mt-0.5">{affiliate.tagline}</p>
          )}
          <p className="text-sm text-[var(--text-muted)] mt-2">{affiliate.description}</p>
        </div>
        <div className="flex-shrink-0 p-2 bg-[var(--emerald-500)]/20 rounded-lg group-hover:bg-[var(--emerald-500)]/30 transition-colors">
          <svg
            className="w-5 h-5 text-[var(--emerald-400)]"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
